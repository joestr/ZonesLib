...
public class ZoneThread implements Runnable {
        ...
        while (currentThread == ZonesLib.zoneThread) {
            try {
                ...
                for (Player player : playerList) {
                    if (!this.leaveTick(player))
                        continue;
                    if (!this.enterTick(player))
                        continue;
    ...
    private boolean leaveTick(Player player) {
        if (!this.resistingList.containsKey(player.getUniqueId()))
            return true;
    ...
    private boolean enterTick(Player player) {
        ...
        for (Zone zone : zoneList) {
            if (this.resistingList.containsKey(player.getUniqueId())
                    && this.resistingList.get(player.getUniqueId()).contains(zone.getId()))
                continue;
            ...
            if (event.isCancelled()) {
                ...
                if (!this.resistingList.containsKey(player.getUniqueId()))
                    this.resistingList.put(player.getUniqueId(), new ArrayList<Integer>());
