...
public class SphereZone extends OriginZone implements RoundedZone {
    ...
    public SphereZone(int zoneId, UUID creator, Timestamp created, boolean deleted, String world, Vector position,
            double radius) {
    ...
    public ZoneBoundingBox getBoundingBox() {
        return new ZoneBoundingBox(this.getX() - this.radius, this.getY() - this.radius, this.getZ() - this.radius,
    ...
    public boolean intersect(Zone zone) {
        ...
        if (zone instanceof SphereZone) {
            ...
            if (cylinderZone.getCenter().getY() + cylinderZone.getHeight() >= this.getCenter().getY() - this.getRadius()
                ...
                if (cylinderZone.getCenter().getY() - cylinderZone.getHeight() <= this.getCenter().getY()
                        && cylinderZone.getCenter().getY() + cylinderZone.getHeight() >= this.getCenter().getY()) {
                    ...
                    radius = Math.sqrt((this.getRadius() * this.getRadius())
                            - ((center.getY() - this.getCenter().getY()) * (center.getY() - this.getCenter().getY())));
                } else {
                    center = this.getCenter().clone();
                    center.setY(cylinderZone.getCenter().getY() - cylinderZone.getHeight());
                    radius = Math.sqrt((this.getRadius() * this.getRadius())
    ...
    public boolean contains(Zone zone) {
        ...
        if (zone instanceof CuboidZone) {
            ...
            lowerRadius = Math
                    .sqrt((this.getRadius() * this.getRadius()) - ((this.getCenter().getY() - lowerCenter.getY())
                            * (this.getCenter().getY() - lowerCenter.getY())));
            upperCenter = this.getCenter().clone();
            upperCenter.setY(cylinderZone.getCenter().getY() + cylinderZone.getHeight());
            upperRadius = Math
                    .sqrt((this.getRadius() * this.getRadius()) - ((this.getCenter().getY() - upperCenter.getY())
                            * (this.getCenter().getY() - upperCenter.getY())));
            lowerCenter.setY(0);
            upperCenter.setY(0);
            Vector cylCenter = cylinderZone.getCenter().clone().setY(0);
