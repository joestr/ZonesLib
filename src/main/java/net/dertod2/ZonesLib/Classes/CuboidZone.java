...
public class CuboidZone extends OriginZone implements HeightZone {
    ...
    public CuboidZone(int zoneId, UUID creator, Timestamp created, boolean deleted, String world, Vector position,
            boolean fullHeight, double height, double width, double length) {
    ...
    public ZoneBoundingBox getBoundingBox() {
        return new ZoneBoundingBox(this.getX(), this.fullHeight ? 128 : this.getY(), this.getZ(), this.length,
                this.fullHeight ? 128 : this.height, this.width);
    }
    ...
    public Area asArea() {
        ...
        return new Area(new Rectangle(min.getBlockX(), min.getBlockZ(), max.getBlockX() - min.getBlockX(),
    ...
    public boolean intersect(Zone zone) {
        ...
        if (zone instanceof CuboidZone) {
            ...
            if (this.intersect(sphereZone.getWorld(), sphereZone.getCenter()))
                return true;

            ZoneLine[] zoneLines = this.getBoundingBox().getCuboidLines();
            for (int i = 0; i < zoneLines.length; i++) {
                if (sphereZone.getCenter().distance(zoneLines[i].getOpposite(sphereZone.getCenter())) <= sphereZone
                        .getRadius()) {
                    return true;
                }
            }
            ...
            if ((this.isFullHeight() || cylinderZone.isFullHeight()) || (cylinderZone.getCenter().getY()
                    + cylinderZone.getHeight() >= this.getCenter().getY() - this.height
                    && cylinderZone.getCenter().getY() - cylinderZone.getHeight() <= this.getCenter().getY()
                            + this.height)) {
