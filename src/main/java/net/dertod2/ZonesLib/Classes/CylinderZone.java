...
public class CylinderZone extends OriginZone implements HeightZone, RoundedZone {
	...
	public CylinderZone(int zoneId, UUID creator, Timestamp created, boolean deleted, String world, Vector position, boolean fullHeight, double height, double radius) {
	...
	public ZoneBoundingBox getBoundingBox() {
		return new ZoneBoundingBox(this.getX() - this.radius, this.isFullHeight() ? 128 : this.getY() - this.height, this.getZ() - this.radius, this.radius * 2, this.isFullHeight() ? 128 : this.height * 2, this.radius * 2);
	}
	...
	public boolean intersect(World world, Vector position) {
		...
		if (this.fullHeight || Zone.isBetween(position.getBlockY(), this.getY() - this.height, this.getY() + this.height)) return true;		
		return false;
	}
	...
	public boolean intersect(Zone zone) {
		...
		if (zone instanceof CylinderZone) {
			...
			if (this.getCenter().distance(cylinderZone.getCenter()) > (this.getRadius() + cylinderZone.getRadius())) return false;
			
			if (this.fullHeight || cylinderZone.fullHeight) return true;	
			
			if (Math.abs(cylinderZone.getY() - this.getY()) > (cylinderZone.height * 2)) return false;
	...
	public boolean contains(Zone zone) {
		...
		if (zone instanceof CuboidZone) {
			...
			if(this.getCenter().getY() + this.getHeight() >= sphereZone.getCenter().getY() + sphereZone.getRadius() && 
					this.getCenter().getY() - this.getHeight() <= sphereZone.getCenter().getY() - sphereZone.getRadius()) {
			...
			if(this.getCenter().getY() + this.getHeight() >= cylinderZone.getCenter().getY() + cylinderZone.getHeight() && 
					this.getCenter().getY() - this.getHeight() <= cylinderZone.getCenter().getY() - cylinderZone.getHeight()) {
