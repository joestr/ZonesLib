...
public abstract class Zone {
	...
	/**
	 * Get all existing {@link Zone}s found at the {@link Position} at the given {@link World}.
	 * @param location The {@link Location} for searching {@link Zone}s
	...
	/**
	 * Get all {@link Zone}s found at the {@link Position} at the given {@link World}.
	 * @param location The {@link Location} for searching {@link Zone}s
	 * @param includeDeleted Whenever deleted zones should be included
	 * @return A {@link List} contains all found {@link Zone}s
	 */
	public static List<Zone> getZones(Location location, boolean includeDeleted) {
		return ZonesLib.getControl().getZones(location.getWorld(), new Vector(location.getX(), location.getY(), location.getZ()), includeDeleted);
	}
	
	/**
	 * Searches for all existing {@link Zone}s colliding with the given {@link Zone}.
	 * @param zone
	...
	/**
	 * Creates an new {@link Zone}'s Class with the {@link ZoneType} specified as the last argument<br />
	 * @param player The {@link OfflinePlayer} or {@link Player} that created the zone
	 * @param positions A List of all needed Positions to create the Zone. The minimum needed Points and maximum allowed points are defined in {@link ZoneType}
	 * @param fullHeight Defines if the new zone should be over the full Y coordinates or not
	 * @param zoneType The Type of this Zone. {@link ZoneType#CUBOID}, {@link ZoneType#CYLINDER}, {@link ZoneType#SPHERE} or {@link ZoneType#POLYGON}.
	 * @return
	 */
	...
	/**
	 * Creates an new TEST {@link Zone}'s Class with the {@link ZoneType} specified as the last argument<br />
	 * @param player The {@link OfflinePlayer} or {@link Player} that created the zone
	 * @param positions A List of all needed Positions to create the Zone. The minimum needed Points and maximum allowed points are defined in {@link ZoneType}
	 * @param fullHeight Defines if the new zone should be over the full Y coordinates or not
	 * @param zoneType The Type of this Zone. {@link ZoneType#CUBOID}, {@link ZoneType#CYLINDER}, {@link ZoneType#SPHERE} or {@link ZoneType#POLYGON}.
	 ...
	 * Can be {@link ZoneType#CUBOID}, {@link ZoneType#CYLINDER} or {@link ZoneType#SPHERE}
	...
	public OfflinePlayer getCreator() {
		...
		if (player == null) player = Bukkit.getOfflinePlayer(this.creator);
	 ...
	 * @param location Location for intersect check
	...
	/**
	 * Checks if this {@link Zone} class intersects with the given {@link World} and {@link ZoneVector}.
	 ...
	 * @param zone The Zone to check if colliding
	 * @return true when colliding or false when not colliding
	 */
	...
	/**
	 * Gets a {@link List} of all {@link ChunkPosition}s inside this {@link Zone} class.
	 * @return A {@link List} contains all {@link ChunkPosition}s
	 */
	public abstract List<ChunkPosition> getChunks();
	
	/**
	 * Gets a {@link List} with all Entities with the given class type inside this zone
	 * @param entities
	...
	public List<Entity> getEntities(@SuppressWarnings("unchecked") Class<? extends Entity>... entities) {
		...
		for (Entity entity : classEntities) {
			if (this.intersect(entity.getLocation())) entityList.add(entity);
		}
	...
	public static boolean isBetween(double point, double a, double b) {
		if ((point >= a && point <= b) || (point <= a && point >= b)) return true;
		return false;
	}
	
	/**
	 * Calls an new ZoneInfoEvent to fetch specific Plugin related Informations to this Zone Object.<br />
