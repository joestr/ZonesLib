...
public class ZoneBoundingBox {
    ...
    public ZoneLine[] getCuboidLines() {
        ...
        zoneLines[0] = new ZoneLine(new Vector(this.x - this.length, this.y - this.height, this.z - this.width),
                new Vector(this.length * 2, 0, 0));
        zoneLines[1] = new ZoneLine(new Vector(this.x - this.length, this.y - this.height, this.z - this.width),
                new Vector(0, this.height * 2, 0));
        zoneLines[2] = new ZoneLine(new Vector(this.x - this.length, this.y - this.height, this.z - this.width),
                new Vector(0, 0, this.width * 2));

        zoneLines[3] = new ZoneLine(new Vector(this.x - this.length, this.y + this.height, this.z + this.width),
                new Vector(this.length * 2, 0, 0));
        zoneLines[4] = new ZoneLine(new Vector(this.x - this.length, this.y + this.height, this.z + this.width),
                new Vector(0, -this.height * 2, 0));
        zoneLines[5] = new ZoneLine(new Vector(this.x - this.length, this.y + this.height, this.z + this.width),
                new Vector(0, 0, -this.width * 2));

        zoneLines[6] = new ZoneLine(new Vector(this.x + this.length, this.y + this.height, this.z - this.width),
                new Vector(-this.length * 2, 0, 0));
        zoneLines[7] = new ZoneLine(new Vector(this.x + this.length, this.y + this.height, this.z - this.width),
                new Vector(0, -this.height * 2, 0));
        zoneLines[8] = new ZoneLine(new Vector(this.x + this.length, this.y + this.height, this.z - this.width),
                new Vector(0, 0, this.width * 2));

        zoneLines[9] = new ZoneLine(new Vector(this.x + this.length, this.y - this.height, this.z + this.width),
                new Vector(-this.length * 2, 0, 0));
        zoneLines[10] = new ZoneLine(new Vector(this.x + this.length, this.y - this.height, this.z + this.width),
                new Vector(0, this.height * 2, 0));
        zoneLines[11] = new ZoneLine(new Vector(this.x + this.length, this.y - this.height, this.z + this.width),
                new Vector(0, 0, -this.width * 2));
    ...
    public ZoneLine[] getSquareLines() {
        ...
        zoneLines[0] = new ZoneLine(new Vector(this.x - this.length, 0, this.z - this.width),
                new Vector(this.length * 2, 0, 0));
        zoneLines[1] = new ZoneLine(new Vector(this.x - this.length, 0, this.z - this.width),
                new Vector(0, 0, this.width * 2));

        zoneLines[2] = new ZoneLine(new Vector(this.x - this.length, 0, this.z + this.width),
                new Vector(this.length * 2, 0, 0));
        zoneLines[3] = new ZoneLine(new Vector(this.x - this.length, 0, this.z + this.width),
                new Vector(0, 0, -this.width * 2));

        zoneLines[4] = new ZoneLine(new Vector(this.x + this.length, 0, this.z - this.width),
                new Vector(-this.length * 2, 0, 0));
        zoneLines[5] = new ZoneLine(new Vector(this.x + this.length, 0, this.z - this.width),
                new Vector(0, 0, this.width * 2));

        zoneLines[6] = new ZoneLine(new Vector(this.x + this.length, 0, this.z + this.width),
                new Vector(-this.length * 2, 0, 0));
        zoneLines[7] = new ZoneLine(new Vector(this.x + this.length, 0, this.z + this.width),
                new Vector(0, 0, -this.width * 2));
    ...
    public boolean intersect(ZoneBoundingBox boundingBox) {
        ...
        if (themXLow > thisXHigh)
            return false;
        if (themYLow > thisYHigh)
            return false;
        if (themZLow > thisZHigh)
            return false;
        if (themXHigh < thisXLow)
            return false;
        if (themYHigh < thisYLow)
            return false;
        if (themZHigh < thisZLow)
            return false;
