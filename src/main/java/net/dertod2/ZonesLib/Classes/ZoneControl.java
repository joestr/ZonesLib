...
public class ZoneControl {
    ...
    public void load() {
        ...
        if (this.zoneList == null)
            this.zoneList = new HashMap<Integer, Zone>();
        if (this.chunkList == null)
            this.chunkList = new HashMap<String, Map<Integer, Map<Integer, List<Integer>>>>();
    ...
    protected Zone add(OfflinePlayer player, List<Location> positions, boolean fullHeight, ZoneType zoneType,
            boolean onlyTemporary) {
        if (positions.isEmpty() || positions.size() < zoneType.getMinimum()
                || (zoneType.getMaximum() != -1 && positions.size() > zoneType.getMaximum()))
            return null;
        ...
        case CUBOID:
            ...
            zone = new CuboidZone(-1, player.getUniqueId(), currentTime, false, world, center, fullHeight, height,
                    width, length);
            break;
        case CYLINDER:
            ...
            zone = new CylinderZone(-1, player.getUniqueId(), currentTime, false, world, center, fullHeight, height,
                    radius);
            break;
        case POLYGON:
            ...
            zone = new PolygonZone(-1, player.getUniqueId(), currentTime, false, world, fullHeight, centerY,
                    maxY - centerY, polysX, polysZ);
            break;
        case SPHERE:
            first = positions.get(0).toVector();
            second = positions.get(1).toVector();
    ...
    private void loadToChunkList(Zone zone) {
        ...
        int start[] = { (int) (zoneBoundingBox.getX() - zoneBoundingBox.getLength()) / 16,
                (int) (zoneBoundingBox.getZ() - zoneBoundingBox.getWidth()) / 16 };
        int end[] = { (int) (zoneBoundingBox.getX() + zoneBoundingBox.getLength()) / 16,
                (int) (zoneBoundingBox.getZ() + zoneBoundingBox.getWidth()) / 16 };

        for (int x = start[0]; x <= end[0]; ++x) {
            for (int z = start[1]; z <= end[1]; ++z) {
                if (!this.chunkList.containsKey(zone.world))
                    this.chunkList.put(zone.world, new HashMap<Integer, Map<Integer, List<Integer>>>());
                if (!this.chunkList.get(zone.world).containsKey(x))
                    this.chunkList.get(zone.world).put(x, new HashMap<Integer, List<Integer>>());
                if (!this.chunkList.get(zone.world).get(x).containsKey(z))
                    this.chunkList.get(zone.world).get(x).put(z, new ArrayList<Integer>());
    ...
    private List<Zone> getChunkZones(World world, Vector position, boolean includeDeleted) {
        ...
        if (!this.chunkList.containsKey(world.getName()))
            return zoneList;
        if (!this.chunkList.get(world.getName()).containsKey(chunkX))
            return zoneList;
        if (!this.chunkList.get(world.getName()).get(chunkX).containsKey(chunkZ))
            return zoneList;
        if (this.chunkList.get(world.getName()).get(chunkX).get(chunkZ) == null)
            return zoneList;

        for (Integer zoneId : this.chunkList.get(world.getName()).get(chunkX).get(chunkZ)) {
            Zone zone = this.zoneList.get(zoneId);
            if (!includeDeleted || zone.isDeleted())
                zoneList.add(this.zoneList.get(zoneId));
        }
        ...
        for (int x = start[0]; x <= end[0]; ++x) {
            for (int z = start[1]; z <= end[1]; ++z) {
                ...
                for (Integer zoneId : this.chunkList.get(zone.world).get(x).get(z)) {
                    ...
                    if (checkZone != null && !zoneList.contains(checkZone) && (!includeDeleted || zone.isDeleted()))
                        zoneList.add(checkZone);
                }
            }
        }
