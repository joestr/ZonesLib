...
public class PolygonZone extends Zone implements HeightZone {
	...
	public PolygonZone(int zoneId, UUID creator, Timestamp created, boolean deleted, String world, boolean fullHeight, double heightY, double height, List<Double> polysX, List<Double> polysZ) {
	...
	public List<Double> getPolysX() {
		...
		for (Vector vector : this.polygonList) polyList.add(vector.getX());
	...
	public List<Double> getPolysZ() {
		List<Double> polyList = new ArrayList<Double>();
		for (Vector vector : this.polygonList) polyList.add(vector.getZ());
	...
	public Area asArea() {
		if (this.cachedArea != null) return this.cachedArea;
	...
	public ZoneBoundingBox getBoundingBox() {
		...
		return new ZoneBoundingBox(center.getX(), this.isFullHeight() ? 128 : center.getY(), center.getZ(), this.maximum.getX() - center.getX() , this.isFullHeight() ? 128 : this.maximum.getY() - center.getY(), this.maximum.getZ() - center.getZ());
	}
	...
	public boolean intersect(Zone zone) { // Relatively easy - use this at any position?
		...
		if (!this.isFullHeight() || !(zone instanceof HeightZone) || (zone instanceof HeightZone && !((HeightZone) zone).isFullHeight())) {
	...
	public boolean contains(Zone zone) {
		...
		if (!this.isFullHeight() || !(zone instanceof HeightZone) || (zone instanceof HeightZone && !((HeightZone) zone).isFullHeight())) {
			if (!Zone.isBetween(this.getMinimum().getY(), zone.getMinimum().getY(), zone.getMaximum().getY())) return false;
			if (!Zone.isBetween(this.getMaximum().getY(), zone.getMinimum().getY(), zone.getMaximum().getY())) return false;
		}
