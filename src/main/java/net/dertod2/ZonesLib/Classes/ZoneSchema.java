...
public class ZoneSchema extends TableEntry {
	...
	public static ZoneSchema toSchema(Zone zone) {
		...
		zoneSchema.centerY = zone instanceof OriginZone ? ((OriginZone) zone).getY() : zone instanceof PolygonZone ? ((PolygonZone) zone).getY() : 0;
	...
	public Zone toZone() {
		...
		switch (zoneType) {
			case CUBOID:
				return new CuboidZone(this.id, UUID.fromString(this.creator), this.created, this.deleted, this.world, position, this.fullHeight, this.height, this.width, this.length);
			case CYLINDER:
				return new CylinderZone(this.id, UUID.fromString(this.creator), this.created, this.deleted, this.world, position, this.fullHeight, this.height, this.radius);
			case SPHERE:
				return new SphereZone(this.id, UUID.fromString(this.creator), this.created, this.deleted, this.world, position, this.radius);
			case POLYGON:
				return new PolygonZone(this.id, UUID.fromString(this.creator), this.created, this.deleted, this.world, this.fullHeight, this.centerY, this.height, this.polysX, this.polysZ);
		}
