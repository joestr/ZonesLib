...
public class AreaDisplayThread implements Runnable {
	...
	protected AreaDisplayThread(Player player, List<Location> locations, ParticleEffect particleEffect, long displayLength) {
		this.player = player;
	...
	public void run() {
		if (this.player != null && this.player.isOnline()) {
			for (Location location : this.locations) {
				...
				if (location.distance(this.player.getLocation()) > 16.0D) continue;
				
				this.particleEffect.display(0F, 0F, 0F, 0F, 1, location, this.player);
			}
		}
		
		if ((displayLength > 0 && displayLength < System.currentTimeMillis()) || this.player == null || !this.player.isOnline()) {
