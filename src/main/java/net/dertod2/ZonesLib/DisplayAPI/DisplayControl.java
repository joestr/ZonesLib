...
public class DisplayControl {
    ...
    public static Map<Player, Integer> startViewing(Zone zone, ParticleEffect particleEffect, long milliseconds,
            Player... playerList) {
        return DisplayControl.startViewing(zone.getFilter().getOuterLine(), particleEffect, milliseconds, playerList);
    }

    public static Map<Player, Integer> startViewing(List<Location> locations, ParticleEffect particleEffect,
            long milliseconds, Player... playerList) {
        ...
        for (Player player : playerList) {
            if (!DisplayControl.displayList.containsKey(player.getUniqueId()))
                DisplayControl.displayList.put(player.getUniqueId(), new ArrayList<Integer>());

            AreaDisplayThread areaDisplayThread = new AreaDisplayThread(player, locations, particleEffect,
                    displayLength);
            int taskId = Bukkit.getScheduler()
                    .runTaskTimerAsynchronously(ZonesLib.getInstance(), areaDisplayThread, 10L, 5L).getTaskId();
    ...
    public static int startViewing(List<Location> locations, ParticleEffect particleEffect, long milliseconds,
            Player player) {
        long displayLength = milliseconds > 0 ? System.currentTimeMillis() + milliseconds : 0;
    ...
    public static boolean stopViewing(Player player, Integer taskId) {
        if (!DisplayControl.displayList.containsKey(player.getUniqueId()))
            return false;
        if (taskId != null && !DisplayControl.displayList.get(player.getUniqueId()).remove((taskId)))
            return false;
        ...
        if (taskId == null) {
            ...
            for (Integer taskListId : taskList)
                Bukkit.getScheduler().cancelTask(taskListId);
        } else {
